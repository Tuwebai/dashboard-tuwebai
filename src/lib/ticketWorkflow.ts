...
class TicketWorkflowService {
  ...
  private async initializeWorkflows() {
    try {
      const workflowsSnapshot = await getDocs(collection(firestore, 'ticketWorkflows'));
      if (workflowsSnapshot.empty) {
        // Crear workflows por defecto
        const batch = writeBatch(firestore);
        DEFAULT_WORKFLOWS.forEach(workflow => {
          const workflowRef = doc(collection(firestore, 'ticketWorkflows'));
          batch.set(workflowRef, workflow);
        });
        await batch.commit();
        this.workflows = DEFAULT_WORKFLOWS;
      } else {
        this.workflows = workflowsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as TicketWorkflow));
      }
    } catch (error) {
  ...
  public async getWorkflows(): Promise<TicketWorkflow[]> {
    try {
      const snapshot = await getDocs(collection(firestore, 'ticketWorkflows'));
      return snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as TicketWorkflow));
    } catch (error) {
  ...
  public async createWorkflow(workflow: Omit<TicketWorkflow, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    try {
      ...
      const docRef = await addDoc(collection(firestore, 'ticketWorkflows'), workflowData);
      return docRef.id;
    } catch (error) {
  ...
  public async updateWorkflow(workflowId: string, updates: Partial<TicketWorkflow>): Promise<void> {
    try {
      const workflowRef = doc(firestore, 'ticketWorkflows', workflowId);
  ...
  public subscribeToWorkflows(callback: (workflows: TicketWorkflow[]) => void): () => void {
    return onSnapshot(collection(firestore, 'ticketWorkflows'), (snapshot) => {
      const workflows = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as TicketWorkflow));
      callback(workflows);
    });
  }
}

...
image.pngexport default function VerDetallesProyecto({ proyecto, onClose, onUpdate }: VerDetallesProyectoProps) {
  const { user } = useApp();
    ...
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-[95vw] sm:max-w-[90vw] md:max-w-[85vw] lg:max-w-[80vw] xl:max-w-[75vw] 2xl:max-w-[70vw] h-[95vh] sm:h-[90vh] overflow-hidden border border-slate-200/50">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-6 sm:p-8 border-b border-slate-200/50 bg-gradient-to-br from-slate-50 via-blue-50/30 to-indigo-50/30 gap-4">
          ...
          <div className="flex items-center gap-3">
            <Button onClick={onClose} variant="ghost" size="sm" className="text-slate-600 hover:bg-slate-100 hover:text-slate-800 p-2">
              <X className="h-6 w-6" />
            </Button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-hidden">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full">
            <div className="border-b border-slate-200/50 px-4 sm:px-6 lg:px-8 bg-slate-50/50">
              <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 bg-white border border-slate-200/50 rounded-xl p-1 h-12">

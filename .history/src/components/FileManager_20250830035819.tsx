...
export default function FileManager({ projectId, isAdmin }: FileManagerProps) {
  ...
  const getFilePreviewUrl = async (file: ProjectFile): Promise<string> => {
    try {
      // Usar URLs firmadas en lugar de URLs públicas para evitar problemas de permisos
      ...
      if (error) {
        ...
        console.log('🔄 Fallback a URL pública:', publicUrl);
        return publicUrl;
      }
      ...
      try {
        ...
        const publicUrl = `${supabaseUrl}/storage/v1/object/public/${bucketName}/${filePath}`;
        console.log('🔄 Fallback final a URL pública:', publicUrl);
        return publicUrl;
      } catch (fallbackError) {

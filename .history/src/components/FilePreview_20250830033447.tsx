...
que permita syntax highlighting y mejor legibilidad.`;
  ...
  // Posicionamiento inteligente (arriba y responsive)
  const getPosition = () => {
    const viewport = {
    ...
    // Tamaño responsive del preview
    const previewWidth = Math.min(400, viewport.width - 40);
    const previewHeight = Math.min(300, viewport.height * 0.4);
    const margin = 20;
    
    // Posición base: arriba del cursor, centrado horizontalmente
    let x = position.x - previewWidth / 2;
    let y = position.y - previewHeight - margin;

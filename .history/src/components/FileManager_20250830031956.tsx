...
export default function FileManager({ projectId, isAdmin }: FileManagerProps) {
  ...
  const getFilePreviewUrl = async (file: ProjectFile): Promise<string> => {
    try {
      ...
      // IMPORTANTE: Usar file.path completo que incluye el nombre real del archivo
      // file.path ya contiene: projectId/nombre_completo_del_archivo
      let filePath = file.path;
      
      // Si file.path no existe o está vacío, construir manualmente
      ...
      console.log('🛤️ Path completo:', file.path);
      return publicUrl;
    } catch (error) {
          ...
          {loading ? (
            ...
            <div className={viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4' : 'space-y-2'}>
              {sortedFiles.map((file) => (
                                 <div
                   key={file.id}
                   className={`group relative p-3 sm:p-4 rounded-lg border border-slate-200 hover:border-slate-300 hover:bg-slate-50 transition-all cursor-pointer ${

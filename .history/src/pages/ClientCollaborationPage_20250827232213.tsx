...
export default function ClientCollaborationPage() {
  ...
  const [uploadingFile, setUploadingFile] = useState<ProjectFile | null>(null);
      ...
      try {
        ...
        const tasksChannel = supabase
          ...
          .on('postgres_changes', 
            ...
            (payload) => {
              ...
              loadTasks(); // Recargar tareas cuando hay cambios
            }
          )
          .subscribe();
        ...
        const loadFiles = async () => {
          ...
          if (filesData) {
            ...
            setCollaborationStats(prev => ({ ...prev, filesUploaded: formattedFiles.length }));
          }
        };
        ...
        const filesChannel = supabase
          ...
          .on('postgres_changes', 
            ...
            (payload) => {
              console.log('Cambio en archivos:', payload);
              loadFiles(); // Recargar archivos cuando hay cambios
            }
          )
          .subscribe();
        ...
        const loadComments = async () => {
          ...
          if (commentsData) {
            ...
            setCollaborationStats(prev => ({ ...prev, totalComments: formattedComments.length }));
          }
        };
        ...
        const commentsChannel = supabase
          ...
          .on('postgres_changes', 
            ...
            (payload) => {
              ...
              loadComments(); // Recargar comentarios cuando hay cambios
            }
          )
          .subscribe();
  ...
  // Helper function to get current unread count
  const getCurrentUnreadCount = async (roomId: string): Promise<number> => {
    try {
    ...
    try {
      ...
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          unreadCount: (await getCurrentUnreadCount(chatRoomId)) + 1
        })
        .eq('id', chatRoomId);
      
      if (updateError) {
        console.error('Error actualizando sala de chat:', updateError);
      }

...
export default function Dashboard() {
  ...
  return (
    <>
      ...
      {/* Vista de proyectos */}
      {!hasValidProjects ? (
        <div className="bg-white rounded-2xl p-6 sm:p-8 lg:p-12 text-center shadow-lg border border-slate-200/50">
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-3 mb-3">
                        <CardTitle className="text-lg sm:text-xl lg:text-2xl font-bold text-slate-800">{project.name || 'Proyecto sin nombre'}</CardTitle>
                        <div className="flex flex-wrap gap-2">
                          <Badge className={getStatusColor(status)}>{status}</Badge>
                          <Badge variant="outline" className="border-slate-300 text-slate-600">{project.type || 'Sin tipo'}</Badge>
                        </div>
                      </div>
                      <p className="text-slate-600 text-sm sm:text-base mb-3">{project.description || 'Sin descripción'}</p>
                      
                      {/* Información del creador del proyecto */}
                      <div className="flex items-center gap-2 p-2 sm:p-3 bg-slate-50 rounded-lg border border-slate-200/50">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                      <Button
                        ...
                        className="border-slate-300 text-slate-700 hover:bg-slate-50 transition-all duration-200 hover:scale-105 w-full sm:w-auto"
                      >
                          ...
                          <CardHeader className="pb-3 p-3 sm:p-4">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                              <CardTitle className="text-sm sm:text-base font-semibold flex items-center gap-2 text-slate-800">
                                ...
                                {fase.estado === 'Terminado' && (
                                  <svg className="w-5 h-5 sm:w-6 sm:h-6 text-green-400 animate-bounce" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                                )}
                                {fase.estado === 'En Progreso' && (
                                  <svg className="w-5 h-5 sm:w-6 sm:h-6 text-blue-400 animate-spin-slow" fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" className="opacity-25" /><path d="M4 12a8 8 0 018-8" stroke="currentColor" strokeWidth="4" className="opacity-75" /></svg>
                                )}
                                {fase.estado === 'Pendiente' && (
                                  <svg className="w-5 h-5 sm:w-6 sm:h-6 text-gray-400 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3" /></svg>
                                )}
                                <span>{faseConfig?.icon || '📋'}</span>
                                <span className="text-xs sm:text-sm">{faseConfig?.label || fase.descripcion || 'Fase'}</span>
                              </CardTitle>
                              <Badge 
                                variant="outline" 
                                className={
                            ...
                            {fase.descripcion && (
                              <p className="text-xs text-slate-500 mt-2">{fase.descripcion}</p>
                            )}
                            {fase.fechaEntrega && (
      ...
      {hasValidProjects && dashboardStats.recentActivity.length > 0 && (
        <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-lg border border-slate-200/50">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4 sm:mb-6">
            <h2 className="text-lg sm:text-xl font-semibold text-slate-800">Actividad Reciente</h2>
            <Button variant="outline" size="sm" className="border-slate-300 text-slate-700 hover:bg-slate-50 w-full sm:w-auto">
          ...
          <div className="space-y-3 sm:space-y-4">
            {dashboardStats.recentActivity.map((project, index) => {
              ...
              return (
                <div key={project.id} className="p-3 sm:p-4 border border-slate-200 rounded-lg bg-slate-50">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
                    <h3 className="font-medium text-slate-800 text-sm sm:text-base">{project.name}</h3>
                  ...
                  {recentComments.length > 0 ? (
                    <div className="space-y-2">
                      {recentComments.map((comment, commentIndex) => (
                        <div key={commentIndex} className="flex items-start gap-3 p-2 sm:p-3 bg-white rounded border border-slate-200">
                          ...
                          <div className="flex-1">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mb-1">
                              <span className="text-xs sm:text-sm font-medium text-slate-800">{comment.autor}</span>
                              <Badge variant="outline" className="text-xs w-fit">
                                {comment.tipo === 'admin' ? 'Admin' : 'Cliente'}
                              </Badge>
                            </div>
                            <p className="text-xs sm:text-sm text-slate-600">{comment.texto}</p>
                    ...
                    <p className="text-xs sm:text-sm text-slate-500 italic">No hay comentarios recientes</p>
                  )}
                </div>
              );
            })}
      ...
      {hasValidProjects && (
        <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-lg border border-slate-200/50">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4 sm:mb-6">
            <h2 className="text-lg sm:text-xl font-semibold text-slate-800">Resumen del Dashboard</h2>
            <div className="flex items-center gap-2 text-xs sm:text-sm">
              <span className="text-slate-600">Última actualización:</span>
              <span className="font-medium text-slate-800">{lastUpdate.toLocaleString()}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <div className="p-3 sm:p-4 bg-blue-50 rounded-lg border border-blue-200">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-blue-800 text-sm sm:text-base">Objetivo</span>
              </div>
              <p className="text-xl sm:text-2xl font-bold text-blue-800">{dashboardStats.averageProgress}%</p>
              <p className="text-xs sm:text-sm text-blue-600">Progreso promedio</p>
            </div>
            
            <div className="p-3 sm:p-4 bg-green-50 rounded-lg border border-green-200">
              <div className="flex items-center gap-2 mb-2">
                <Award className="h-4 w-4 sm:h-5 sm:w-5 text-green-600" />
                <span className="font-medium text-green-800 text-sm sm:text-base">Éxito</span>
              </div>
              <p className="text-xl sm:text-2xl font-bold text-green-800">{dashboardStats.completedProjects}</p>
              <p className="text-xs sm:text-sm text-blue-600">Proyectos completados</p>
            </div>
            ...
            <div className="p-3 sm:p-4 bg-amber-50 rounded-lg border border-amber-200">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-medium text-amber-800 text-sm sm:text-base">Actividad</span>
              </div>
              <p className="text-xl sm:text-2xl font-bold text-amber-800">{dashboardStats.totalComments}</p>
              <p className="text-xs sm:text-sm text-amber-600">Comentarios totales</p>
            </div>
            
            <div className="p-3 sm:p-4 bg-purple-50 rounded-lg border border-purple-200">
              <div className="flex items-center gap-2 mb-2">
                <Star className="h-4 w-4 sm:h-5 sm:w-5 text-purple-600" />
                <span className="font-medium text-purple-800 text-sm sm:text-base">Calidad</span>
              </div>
              ...
              <p className="text-xs sm:text-sm text-purple-600">Tasa de finalización</p>
            </div>
          </div>
        </div>
      )}
